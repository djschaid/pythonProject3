...
## added gigrnd_vec to receive same length vectors a and b and
## return vec of same length (Dan Schaid)
import math
...
def gigrnd_vec(p, a, b):
    if len(a) != len(b):
        print("error: vec input to gigrnd_vec not equal length")

    n = len(a)
    result = np.zeros(n)
    for i in range(n):
        result[i] = gigrnd(p, a[i], b[i])
    return result
